...
The new year 2023 started with the release of a new version of the Azure Developer CLI to version [0.5.0-beta3](https://github.com/Azure/azure-dev/releases/tag/azure-dev-cli_0.5.0-beta.1) that brought several updates. Several of those updates are from my perspective cleaning and leaning up things under the hood, but they also impact some artifacts of the setup.
...
I will not walk through each and every change but highlight the ones that I think are most important when updating your existing `azd`-compatible repository. Therefore I highly recommend to walk-through the release note yourself as there might be gems you are waiting for.
...
and then install it which did the trick. I am not sure if this is connected to the PR [#1311](https://github.com/Azure/azure-dev/pull/1311) (Add support to install script with MSI on Windows) but something that went different this time.

After that the updates to the beta versions 0.5.0-beta2 and 0.5.0-beta3 worked as before, so everything is fine again.

> 📝 Remark: If you want to uninstall versions > 0.5.0 you must use the `Add/Remove programs`way in Windows.

### The new logon experience

Until version 0.4.0-beta.1 the log on to Azure was executed via a logon using the Azure CLI. This was a bit of a break in the flow of the user first using the Azure CLI and then `azd`. This break was removed with version 0.5.0-beta1, so you can now use the new command:

```powershell
azd login
```

to log into your Azure subscription. This makes the experience smoother and more self contained, but also has an impact to the setup of the CI/CD artifacts provided by `azd`. You can switch back to the old behavior of `azd` by the following configuration:

```powershell
azd config set auth.useAzCliAuth true
```

> **Attention**: there was a bug in version 0.5.0-beta.1 when you tried to logon to a personal account. This failed, but is fixed with version [0.5.0-beta.2](https://github.com/Azure/azure-dev/releases/tag/azure-dev-cli_0.5.0-beta.2). However now the `azd init` command does not show any subscriptions, see issue [1398](https://github.com/Azure/azure-dev/issues/1398) when using the new logon flow. So for the time being I recommend to use the fallback behavior for personal accounts.
...
For the GitHub Action the logon part must be completely exchanged and has the following structure now:
...
In contrast to the release note the templates provide two flow options to logon to azure via `azd` leveraging the new options that come with the new command. The second option with the *client credentials* that represents the exact replacement of the old step with the new flow.
...
With this release the infrastructure setup *per se* remained stable. But we can welcome two new templates for the infrastructure. The first one is also coupled to a new category, namely one for *Azure Api Management* that you find now in the `gateway` folder:

![azd apim template](./assets/infra-apim-template.png)

The second one (and I can proudly say that I contributed that one via this [PR](https://github.com/Azure/azure-dev/pull/1197), however did not get a mention in the official release blog post 😢) is that the *Key Vault Secrets* are now available as part of the default setup. Due to the nature of the secret string that needs to be constructed manually we still need an own module to do the job.
I removed the `core_local` folder with its content and created a new module in the `app` folder called `blobStorageConnectionSecret.bicep`:
...
The refactoring in release 0.5.0-beta.1 of the `azd` removed the dependency of the Azure CLI (see PR [1129](https://github.com/Azure/azure-dev/pull/1129)). This has an impact on the setup of the development container and should lead to a faster first startup. The new Dockerfile has the following content now:
...
The `xdg-utils` is now installed as an additional package.
...
> **Attention**: As mentioned before the fetching of subscriptions for personal accounts in `azd init` is broken up to release [0.5.0-beta.3](https://github.com/Azure/azure-dev/releases/tag/azure-dev-cli_0.5.0-beta.2) as reported in issue [1398](https://github.com/Azure/azure-dev/issues/1398). If you are working with a personal account and a devcontainer or GutHub Codespaces you should keep the Azure CLI feature in your `devcontainer.json` like [here](https://github.com/lechnerc77/azd-compatibility-azure-function/blob/azd-040-beta1/.devcontainer/devcontainer.json).
...
It was also aligned across different commands to have a consistent experience. This is appreciated. However some commands could also benefit from an improvement in that area like the output of the command `azd template list`.

### And more

As mentioned in the beginning there are several more additions in the release like [shell completion](https://github.com/Azure/azure-dev/pull/1100), [FederatedIdentityCredentials](https://github.com/Azure/azure-dev/pull/1086) or the new command [azd auth token](https://github.com/Azure/azure-dev/pull/1177). Although all new features and fixes not mentioned here are not relevant to update your 0.4.0-beta.x setup to 0.5.0-beta.x, there might be some features you have been waiting for in there. So check the [release note](https://github.com/Azure/azure-dev/releases/tag/azure-dev-cli_0.5.0-beta.1) and maybe find some more gems you might have been waiting for.  

## Summary

As usual *kudos* to the Azure Developer CLI team and the work they do in pushing the CLI forward. In contrast to the other releases up to now this release contains a lot of refactoring and new core functionality. I like it and I think it is the right direction. Having said that this is also the first release that is a bit more of a bumpy experience compared to the previous ones. If you install this version be aware that there are some rough edges compared to the previous upgrades. The good thing is at least from my experience porting two projects to the new version is that there are workarounds and you do not hit a dead end with any of the bugs. The response time to issues is very fast, but I would assume that there is a bit of room for improvement with respect to regression tests.

So keep it coming `azd`, looking forward to the next iterations!
